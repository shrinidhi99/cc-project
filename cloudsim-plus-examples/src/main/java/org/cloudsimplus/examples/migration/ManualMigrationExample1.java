...
public final class ManualMigrationExample1 {
    ...
    private static final int    VM_RAM = 1000; //VM memory (MB)
    ...
    private static final long   DIRTYING_RATE = 10;
    ...
    private void clockTickListener(EventInfo info) {
        ...
        if(!migrationRequested && info.getTime() >= 10){
            for (int i=0;i<vmList.size();i++){
                ...
                datacenter0.requestVmMigration(sourceVm, targetHost, false, DIRTYING_RATE);
                this.migrationRequested = true;
            }
        }
    }

    private int get_host_for_migration(Vm vm, int index)
    {
        System.out.println("Capacity " + vm.getRam().getCapacity());
        System.out.println("Allocated resource: " + vm.getRam().getAllocatedResource());
        while(index >= 0 && hostList.get(index).getAvailableStorage() < vm.getRam().getCapacity())
            index--;
        if (index < 0)
            return -1;
        return index;
